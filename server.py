...
def index():
	...
	#if the user clicks on one of the button I recive the game_mode through a POST method
...
def singlePlayer():
	...
	if request.method == "POST":
		...
		#I send the cell_id to the python code, and i recive if the game is over and who is the winner
		(is_game_over, player_winner) = ttt.game_mode_web1(cell_id[0], cell_id[1])
...
def multiPlayer():
	...
	if request.method == "POST":
		...
		(is_game_over, player_winner) = ttt.game_mode_web2(cell_id[0], cell_id[1])
